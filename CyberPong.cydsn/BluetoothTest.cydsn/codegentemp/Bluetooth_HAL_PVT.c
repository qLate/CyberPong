...
void CyBLE_Uart_Start (void)
{
        ...
        /* Setup ISR */
        Bluetooth_uart_isr_StartEx(&CyBLE_Uart_Interrupt);

        Bluetooth_HAL_Uart_Start();
...
void CyBLE_Uart_Stop (void)
{
        ...
        Bluetooth_uart_isr_Stop();

        Bluetooth_HAL_Uart_Stop();
    #endif /* (CYBLE_HCI_TYPE == CYBLE_HCI_OVER_UART) */
}
...
{
    #if (CYBLE_HCI_TYPE == CYBLE_HCI_OVER_UART)
        Bluetooth_HAL_Uart_SpiUartPutArray(dataBuf, (uint32)length);
    #endif /* (CYBLE_HCI_TYPE == CYBLE_HCI_OVER_UART) */
}
...
/* Interface to Bluetooth_bless_isr_Enable function */
void CyBleHal_EnableGlobalInterrupts(void)
{
    Bluetooth_bless_isr_Enable();
}

/* Interface to Bluetooth_bless_isr_Disable function */
...
void CyBle_HalInit(void)
{
    ...
    Bluetooth_bless_isr_SetVector(&CyBLE_Bless_Interrupt);
...
void CYBLE_BlessStart(void)
{
    ...
    Bluetooth_bless_isr_StartEx(&CyBLE_Bless_Interrupt);
}
